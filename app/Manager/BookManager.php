<?php 
	...
	class BookManager extends \W\Manager\Manager
	{
		...
		{	
			// LA REQUETE NINJA POWA DYNAMIQUE DE LA MORT
			$sql = "SELECT cat.style AS catstyle, books.cover, books.title, books.id, books.stock, books.publisher, i.lastName AS ilastname, i.firstName AS ifirstname, s.lastName AS slastname, s.firstName AS sfirstname, c.lastName AS clastname, c.firstName AS cfirstname
					FROM $this->table					
					LEFT JOIN authors AS s ON books.scenarist = s.id
					LEFT JOIN authors AS i ON books.illustrator = i.id 
					LEFT JOIN authors AS c ON books.colorist = c.id
					LEFT JOIN series AS t ON books.serieId = t.id
					LEFT JOIN series AS cat ON books.style = cat.id
					WHERE books.id = :id";
					// LA REQUETE DYNAMIQUE S'EXECUTE UNE SEULE FOIS
			$sth = $this->dbh->prepare($sql);
			$sth->bindValue(':id', $id );
